...
void UART_Init(void){ long status = 0;
  ...
  URxFifo_Init();                        // initialize empty FIFOs
  UTxFifo_Init();
...
void static copyHardwareToSoftware(void){ 
  char letter;
  while(((UART0_FR_R&UART_FR_RXFE) == 0) && (URxFifo_Size() < (FIFOSIZE - 1))){
    letter = UART0_DR_R;
    URxFifo_Put(letter);
    //
    OS_Signal(&RxDataAvailable);    // Use for semaphore 
  }
}
...
void static copySoftwareToHardware(void){
  char letter;
  while(((UART0_FR_R&UART_FR_TXFF) == 0) && (UTxFifo_Size() > 0)){
    UTxFifo_Get(&letter);
    UART0_DR_R = letter;
...
char UART_InChar(void){
  char letter;
  OS_Wait(&RxDataAvailable);       // Use for semaphore 
  URxFifo_Get(&letter);            // Use for semaphore 
  //while(URxFifo_Get(&letter) == FIFOFAIL){};
  return(letter);
}
// output ASCII character to UART
// spin if TxFifo is full
void UART_OutChar(char data){ 
  OS_Wait(&TxRoomLeft);           // Use for semaphore 
  UTxFifo_Put(data);              // Use for semaphore 
  //while(UTxFifo_Put(data) == FIFOFAIL){};
...
void UART0_Handler(void){
  if(UART0_RIS_R&UART_RIS_TXRIS){       // hardware TX FIFO <= 2 items
    ...
    // copy from software TX FIFO to hardware TX FIFO
    copySoftwareToHardware();
    if(UTxFifo_Size() == 0){             // software TX FIFO is empty
      UART0_IM_R &= ~UART_IM_TXIM;      // disable TX FIFO interrupt
    }
  }
...
void UART_InString(char *bufPt, uint16_t max) {
  ...
  character = UART_InChar();
  while(character != CR){
		if(character == DEL || character == BS || (character >= 32 && character <= 126)){
		  if(character == DEL){
        if(length > 0){
          bufPt--;
          length--;
...
void UART_printf(const char * format, ...) {
  ...
  vsnprintf(buffer, MAX_STRING_SIZE, format, args);
  va_end(args);
