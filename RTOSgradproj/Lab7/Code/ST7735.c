...
// ***************** ST7735_S Series ********************
// The S stands for split
// These functions separate the ST7735 into two logically different displays - top and bottom.
// These functions were developed by Allen Wang and Alvin Tung
#define DEVICE_OFFSET  80
#define Y_OFF				 	 0
#define CHAR_HEIGHT	   10
#define CHAR_WIDTH 	   6
#define CHAR_SIZE      1
#define TOP_TEXT       ST7735_WHITE
#define BOTTOM_TEXT    ST7735_BLACK
#define TOP_BG         ST7735_BLACK
#define BOTTOM_BG      ST7735_WHITE
#define MAX_CHARS			 20 							// max chars per line

// Used for separate devices
int x_pos[2][8];
Sema4Type ST7735Mutex;
// **************** ST7735_SInit **********************************
// Initializations for S series
// Uses OS_Semaphores
void ST7735_SInit(void) {
  OS_InitSemaphore(&ST7735Mutex, 1);
	ST7735_InitR(INITR_REDTAB);					// Main screen initializer
  ...
  ST7735_FillRect(0, 0, _width, _height / 2, TOP_BG);
  ST7735_FillRect(0, _height / 2, _width, _height / 2, BOTTOM_BG);
}

// Helper function for mostly all S functions
static void SGetParams(int * x, int * y, int *dev_text, int *dev_bg, int device, int line) {
	*y = line * CHAR_HEIGHT;
	*x = x_pos[device][line];
	if (device == ST7735_TOP) {
		*dev_text = TOP_TEXT;
		*dev_bg = TOP_BG;
	}	else if (device == ST7735_BOTTOM) {
		*y += DEVICE_OFFSET;
		*dev_text = BOTTOM_TEXT;
		*dev_bg = BOTTOM_BG;
	} else { 
		return;
	}
...
// **************** ST7735_SClear **********************************
// Separates the ST7735 into two logically different displays
// One display uses the top and the other uses the bottom
// This function clears a specific line
// *** NOTE - THIS FUNCTION IS NOT OS SAFE
int ST7735_SClear(int device, int line) {
  OS_Wait(&ST7735Mutex);
	if ((device != 1 || device != 0)  && (line < 0 || line > 8)) { OS_Signal(&ST7735Mutex); return 0; }
	
	int x, y, dev_text, dev_bg;
	SGetParams(&x, &y, &dev_text, &dev_bg, device, line);
	ST7735_FillRect(0, y, _width, CHAR_HEIGHT, dev_bg);
	x_pos[device][line] = 0;
  OS_Signal(&ST7735Mutex);
	return 1;
}
...
// Separates the ST7735 into two logically different displays
// One display uses the top and the other uses the bottom
// - Device specifies top (1) or bottom (0)
// - Line specifies line number (0 - 7)
// - string is a pointer null terminated message
int ST7735_SDrawString(int device, int line, char * string) {
	OS_Wait(&ST7735Mutex);
  if ((device != 1 || device != 0)  && (line < 0 || line > 8)) { OS_Signal(&ST7735Mutex); return 0; }
	...
	SGetParams(&x, &y, &dev_text, &dev_bg, device, line);
	while (*string) {
		if (x > MAX_CHARS * CHAR_WIDTH) { break; }
		ST7735_DrawChar(x*CHAR_WIDTH, y + Y_OFF, *string, dev_text, dev_bg, CHAR_SIZE);
		x++;
		string++;
	}
	x_pos[device][line] = x;
...
int ST7735_SDrawValue(int device, int line, long value) {
  OS_Wait(&ST7735Mutex);
	if ((device != 1 || device != 0)  && (line < 0 || line > 8)) {  OS_Signal(&ST7735Mutex); return 0; }
	char buf[MAX_DIGITS];
  char * p  = itoa(value, buf, 10);
  ...
  while (p[i]) {
		if (x > MAX_CHARS * CHAR_WIDTH) { break; }
		ST7735_DrawChar(x*CHAR_WIDTH, y + Y_OFF, p[i], dev_text, dev_bg, CHAR_SIZE);
		x++;
		i++;
	}
	x_pos[device][line] = ++x;
...
// **************** ST7735_Message **********************************
// Combines the functionality of drawing a string and a value
// It calls ST7735_SDrawString and ST7735_SDrawValue.
// Therefore it makes both of those functions not OS Safe
// Use ST7735_SMessage for any user calls.
void ST7735_Message(unsigned long d, unsigned long l, char *pt, long value) {
  ST7735_SDrawString(d, l, pt);
  ST7735_SDrawValue(d, l, value);
}
