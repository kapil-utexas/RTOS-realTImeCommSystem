...
void ESP8266_InitUART(uint32_t baud, int echo){ volatile int delay;
  ...
  UART2_IBRD_R = 5000000/baud;   
  UART2_FBRD_R = ((64*(5000000%baud))+baud/2)/baud;      
...
void UART2_Handler(void){
  if(UART2_RIS_R & UART_RIS_RXRIS){   // rx fifo >= 1/8 full
    UART2_ICR_R = UART_ICR_RXIC;      // acknowledge interrupt
    ESP8266FIFOtoBuffer();
  }
  if(UART2_RIS_R & UART_RIS_RTRIS){   // receiver timed out
    UART2_ICR_R = UART_ICR_RTIC;      // acknowledge receiver time
    ESP8266FIFOtoBuffer();
  }
}
...
// step 4: AT+CIPMODE=0 set mode to not data mode
  if(ESP8266_SetDataTransmissionMode(0)==0){ 
    UART_printf("SetDataTransmissionMode, could not make connection\n\r"); while(1){};
  }
// step 4: AT+CIPMODE=0 set mode to not data mode
  if(ESP8266_SetDataTransmissionMode(0)==0){ 
    UART_printf("SetDataTransmissionMode, could not make connection\n\r"); while(1){};
...
int ESP8266_Reset(){int try=MAXTRY;
  SearchStart("ready");
  while(try){
    GPIO_PORTB_DATA_R &= ~0x02; // reset low
    DelayMs(10);
    GPIO_PORTB_DATA_R |= 0x02; // reset high
...
int ESP8266_MakeTCPConnection(char *IPaddress){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CIPSTART=\"TCP\",\"%s\",11111\r\n", IPaddress);
